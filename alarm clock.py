...
class AdvancedAlarmClock:
    def __init__(self, root):
        ...
        self.root.geometry("400x400") # Made slightly taller to fit the audio selector
        self.root.resizable(False, False)
        
        # Initialize Pygame Mixer for audio
        pygame.mixer.init()
        self.sound_file = "" # Starts empty, user must select one
    ...
    def setup_ui(self):
        ...
        # --- NEW: Audio File Selector ---
        audio_frame = tk.Frame(frame)
        audio_frame.pack(pady=10)
        
        self.sound_label = tk.Label(audio_frame, text="Sound: Not Selected", font=("Helvetica", 9), fg="#cc0000")
        self.sound_label.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(audio_frame, text="Browse Audio...", command=self.browse_audio).pack(side=tk.LEFT)
    ...
    def browse_audio(self):
        """Opens a file dialog to select an MP3 or WAV file."""
        ...
        if file_path:
            ...
            # Extract just the file name to keep the UI clean
            file_name = os.path.basename(file_path)
            self.sound_label.config(text=f"Sound: {file_name}", fg="green")

    def update_live_clock(self):
        """Updates the digital clock on the GUI every 1000 milliseconds."""
        current_time = datetime.datetime.now().strftime('%H:%M:%S')
        self.clock_label.config(text=current_time)
        self.root.after(1000, self.update_live_clock)

    def set_alarm(self):
        """Grabs the input, validates the audio file, and starts the thread."""
        # Validation: Ensure a sound file is selected
        if not self.sound_file or not os.path.exists(self.sound_file):
            messagebox.showerror("Missing Audio", "Please click 'Browse Audio...' and select a valid .mp3 or .wav file first!")
            return
        ...
        # Start the checking loop in a separate thread
    ...
    def check_alarm_loop(self):
        ...
        while self.is_alarm_set:
            ...
            if current_time == self.alarm_time:
                # Use root.after to safely trigger UI updates from the background thread
